...
func (it *randomIterator) nextNode() *enode.Node {
	for {
		...
		if err != nil {
			...
			it.c.cfg.Logger.Info("Error in DNS random node sync", "tree", ct.loc.domain, "err", err)
			continue
		}
		if n != nil {
			return n
...
func (it *randomIterator) waitForRootUpdates(trees []*clientTree) bool {
	...
	it.c.cfg.Logger.Info("DNS iterator waiting for root updates", "sleep", sleep, "tree", minTree.loc.domain)
